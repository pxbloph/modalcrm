...
async function run() {
    console.log('--- DETALHAMENTO: LEADS PONTUADOS (RAIANE) ---');
    ...
    // Consulta idêntica à da TV Dashboard, focada na tabela de Deals (responsabilidade)
    const validDeals = await prisma.deal.findMany({
        where: {
            ...
            responsible: {
                name: { contains: 'Raiane', mode: 'insensitive' }
            }
        },
        select: {
            client: {
    ...
    console.log(`\nEncontrados ${validDeals.length} negócios associados à conta da Raiane hoje. Detalhando autoria e responsabilidade:\n`);
