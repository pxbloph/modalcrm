...
export default function PipelinesSettingsPage() {
    ...
    const createPipeline = async () => {
        ...
        try {
            ...
            router.push(`/settings/pipelines/${res.data.id}`);
        } catch (error) {
        ...
        <div className="p-6 space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-muted-foreground">Gerencie seus fluxos de vendas e processos</p>
                </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {pipelines.map((pipeline) => (
                    <Card key={pipeline.id} className="cursor-pointer hover:shadow-md transition-shadow border-border hover:border-primary/50" onClick={() => router.push(`/settings/pipelines/${pipeline.id}`)}>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            <CardTitle className="text-sm font-medium text-foreground">
                                {pipeline.name}
                                {pipeline.is_default && <span className="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full border border-primary/20">Padrão</span>}
                            </CardTitle>
                            <Settings className="h-4 w-4 text-muted-foreground" />
                        </CardHeader>
                        <CardContent>
                            <div className="text-2xl font-bold text-foreground">{pipeline._count?.stages || 0} Etapas</div>
                            <p className="text-xs text-muted-foreground">
                                {pipeline._count?.deals || 0} negócios ativos
                            </p>

                            <div className="mt-4 flex justify-end" onClick={(e) => e.stopPropagation()}>
                                <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => deletePipeline(pipeline.id)}>
