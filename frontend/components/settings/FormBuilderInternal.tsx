...
const SortableField = ({ field, onRemove, onEdit }: SortableFieldProps) => {
    ...
    return (
        <div
            ref={setNodeRef}
            style={style}
            className="group relative bg-white border border-gray-200 rounded-lg p-4 mb-3 hover:border-indigo-300 transition-colors shadow-sm"
        >
                ...
                <div className="flex-1" onClick={() => onEdit(field)}>
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <p className="text-xs text-gray-500 uppercase mt-0.5">{field.type}</p>

                            {field.systemField && (
                                <span className="inline-block mt-1 bg-gray-100 text-gray-600 text-[10px] px-1.5 py-0.5 rounded border border-gray-200">
                    ...
                    {(field.type === 'select' || field.type === 'radio') && field.options && field.options.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1">
                            {field.options.map((opt, idx) => (
                                <span key={idx} className="text-xs bg-gray-50 text-gray-600 px-2 py-0.5 rounded border border-gray-100">
                                    {opt.label}
                                </span>
                            ))}
                        </div>
        ...
        <div className="flex gap-6 h-[calc(100vh-200px)]">
            ...
            <div className="flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-lg font-medium text-gray-900">Estrutura do Formulário</h3>
                    <button
                        onClick={addField}
            ...
            {editingField && (
                <div className="w-80 bg-white border-l border-gray-200 p-6 overflow-y-auto shadow-xl fixed right-0 top-0 bottom-0 z-50 md:relative md:z-auto md:shadow-none md:top-auto md:bottom-auto md:h-full">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-semibold text-gray-900">Editar Campo</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Rótulo (Label)</label>
                            <input
                                type="text"
                                className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border text-gray-900"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            />
                            <label htmlFor="req" className="text-sm text-gray-700">Obrigatório</label>
                        </div>
                                ...
                                <div className="space-y-2">
                                    {editingField.options?.map((opt, idx) => (
                                        <div key={idx} className="flex gap-2">
                                            <input
                                                type="text"
                                                className="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-xs p-1.5 border text-gray-900"
                                                value={opt.label}
                                                onChange={e => {
